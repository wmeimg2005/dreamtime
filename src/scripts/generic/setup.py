...
parser.add_argument(
    "-np", "--nopyinstaller", action="store_true", help="Don't install pyinstaller"
)
parser.add_argument("--cpu", action="store_true", help="No cuda support")
...
if do_all or args.cli:
    ...
    path = create_temporary_copy(
        "../../cli/requirements-generic.txt", "cli-requirements.txt"
    )
...
if do_all or args.gui:
    ...
    with cd("../../gui"):
        r = (
            ...
            if detected_os == OS.WIN
            else subprocess.run(["yarn", "install"])
        )
        if r.returncode != 0:
